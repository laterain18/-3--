...
void TIMA1_IRQHandler (void)
{
    ...
    if(ms_count % 5 == 0) // 每 5 次中断触发一次
    {
        flag.ms_5 = 1; // 设置 10 毫秒标志位
    }
    if(ms_count % 10 == 0) // 每 10 次中断触发一次
    {     
        flag.ms_10 = 1; // 设置 10 毫秒标志位
    }
    if(ms_count % 20 == 0) // 每 20 次中断触发一次
    {
        flag.ms_20 = 1; // 设置 20 毫秒标志位
    }
    if(ms_count % 100 == 0) // 每 20 次中断触发一次
    {
        flag.ms_100 = 1; // 设置 20 毫秒标志位
        ms_count = 0; // 重置计数器
    }
}
